...
CREATE OR REPLACE FUNCTION filter_process(process_schema text, process_type_name TEXT, keyword_filter TEXT, start_time timestamp, end_time timestamp)
RETURNS json
AS
$$
DECLARE
  ...
  fts_config text;
begin
       SELECT 'pg_catalog.'||fts_config_from_language(language->>0)::text into fts_config
       FROM lendas_catalog.process_types p
       WHERE p.schema = process_schema and p.name=process_type_name;
       ...
       has_feature_tsvector = EXISTS (SELECT * FROM information_schema.columns
                                      WHERE table_schema = process_schema and table_name=process_type_name
      ...
      ELSEIF has_feature_tsvector AND NOT has_valid_time_tsvector THEN
           keyword_filter_restriction:=format('p.keyword_vector @@ to_tsquery(''%s'', ''%s'')',fts_config, keyword_filter);
      ...
      ELSE   keyword_filter_restriction:=format('(p.keyword_vector || t.keyword_vector) @@ to_tsquery(''%s'', ''%s'')',fts_config, keyword_filter);
      END IF;
    ...
    EXECUTE format('SELECT json_agg(jo)
		FROM 
            ...
            process_schema,process_type_name,join_expression,keyword_filter_restriction,temporal_restriction, group_expression)
	 INTO result_json;
